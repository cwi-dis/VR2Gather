...
public class LogSaver : MonoBehaviour {
    ...
    void Start () {
        ...
        synchronizer = GameObject.FindObjectOfType<VRTSynchronizer>();
    ...
    void Update () {
        ...
        synchronizer = (synchronizer == null) ? GameObject.FindObjectOfType<VRTSynchronizer>() : synchronizer;
        state = (synchronizer == null) ? "waiting" : "session";
        head = UserCamera.transform.eulerAngles;
        position = UserCamera.transform.position;
        delayValue = (state == "waiting") ? 0: synchronizer.minLatency;
