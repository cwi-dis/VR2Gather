...
public class NetworkPlayer : MonoBehaviour
{
	...
	private void Awake()
	{
		_SendDelta = 1.0f / (float)SendRate;
	...
	void SendPlayerData()
	{
		...
		{
			OrchestratorController.Instance.SendTypeEventToAll<NetworkPlayerData>(data);
		}
		else
		{
			OrchestratorController.Instance.SendTypeEventToMaster<NetworkPlayerData>(data);
		}
	...
	void OnNetworkPlayerData(NetworkPlayerData data)
	{
		if (!IsLocalPlayer && UserId == data.SenderId)
		{
			if(OrchestratorController.Instance.UserIsMaster)
			{
				...
				OrchestratorController.Instance.SendTypeEventToAll<NetworkPlayerData>(data, true);
			}
			...
			if (_PreviousReceivedData != null)
			{
				...
				float t = Mathf.Clamp01((Time.realtimeSinceStartup - _LastReceiveTime) / (1.0f / (float)SendRate));
