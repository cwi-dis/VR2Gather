...
public class PlayerTransformSyncBehaviour : MonoBehaviour
{
	...
	private void DoSync()
	{
		...
		{
			OrchestratorController.Instance.SendTypeEventToAll<PlayerTransformSyncData>(data);
		}
		else
		{
			OrchestratorController.Instance.SendTypeEventToMaster<PlayerTransformSyncData>(data);
		}
	}
	...
	void OnPlayerTransformSyncData(PlayerTransformSyncData data)
	{
		if (data.SenderId == Player.UserId)
		{
			if (OrchestratorController.Instance.UserIsMaster)
			{
				OrchestratorController.Instance.SendTypeEventToAll<PlayerTransformSyncData>(data, true);
			}
