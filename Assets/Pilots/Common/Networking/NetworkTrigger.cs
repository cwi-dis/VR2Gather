...
public class NetworkTrigger : NetworkIdBehaviour
{
	...
	public virtual void Trigger()
	{
		...
		{
			OrchestratorController.Instance.SendTypeEventToMaster<NetworkTriggerData>(triggerData);
		}
		else
		{
			OnTrigger.Invoke();
			...
			OrchestratorController.Instance.SendTypeEventToAll<NetworkTriggerData>(triggerData);
		}
	}
	...
	void OnNetworkTrigger(NetworkTriggerData data)
	{
		if(NeedsAction(data.NetworkBehaviourId))
		{
			...
			if(OrchestratorController.Instance.UserIsMaster)
			{
				OrchestratorController.Instance.SendTypeEventToAll<NetworkTriggerData>(data);
			}
		}
	}
