...
namespace Workers {
    ...
    public class CerthReader : TiledWorker   // Doesn't have to be a BaseWorker, but EntityPipeline expects it.
    {
        ...
        public void statsUpdate(int pointCount, bool dropped=false)
        {
            ...
            {
                Debug.Log($"stats: ts={(int)System.DateTime.Now.TimeOfDay.TotalSeconds}, component=CerthReader, fps={statsTotalPointclouds / statsInterval}, points_per_cloud={(int)(statsTotalPoints / statsTotalPointclouds)}, drops_per_second={statsDrops/statsInterval}");
